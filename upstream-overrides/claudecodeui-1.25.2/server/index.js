...
function codexOnlyMiddleware(feature) {
    return (req, res, next) => {
        if (!CODEX_ONLY_HARDENED_MODE) {
            return next();
        }

        return blockDisabledFeature(res, feature);
    };
}
...
const wss = new WebSocketServer({
    ...
    verifyClient: (info) => {
        ...
        if (IS_PLATFORM) {
            const user = authenticateWebSocketRequest(info.req);
            if (!user) {
        ...
        // Normal mode: verify cookie-backed session (with bearer token fallback).
        const user = authenticateWebSocketRequest(info.req);
        if (!user) {
...
app.use('/api/git', authenticateToken, CODEX_ONLY_HARDENED_MODE ? codexOnlyMiddleware('Git routes') : gitRoutes);

// MCP API Routes (protected)
app.use('/api/mcp', authenticateToken, CODEX_ONLY_HARDENED_MODE ? codexOnlyMiddleware('MCP routes') : mcpRoutes);
...
app.use('/api/taskmaster', authenticateToken, CODEX_ONLY_HARDENED_MODE ? codexOnlyMiddleware('TaskMaster routes') : taskmasterRoutes);

// MCP utilities
app.use('/api/mcp-utils', authenticateToken, CODEX_ONLY_HARDENED_MODE ? codexOnlyMiddleware('MCP utility routes') : mcpUtilsRoutes);

// Commands API Routes (protected)
app.use('/api/commands', authenticateToken, CODEX_ONLY_HARDENED_MODE ? codexOnlyMiddleware('Command routes') : commandsRoutes);

// Settings API Routes (protected)
app.use('/api/settings', authenticateToken, CODEX_ONLY_HARDENED_MODE ? codexOnlyMiddleware('Settings routes') : settingsRoutes);
...
app.use('/api/plugins', authenticateToken, CODEX_ONLY_HARDENED_MODE ? codexOnlyMiddleware('Plugin routes') : pluginsRoutes);
...
app.post('/api/system/update', authenticateToken, async (req, res) => {
    if (CODEX_ONLY_HARDENED_MODE) {
        return blockDisabledFeature(res, 'System update route');
    }
...
app.get('/api/projects/:projectName/sessions', authenticateToken, async (req, res) => {
    if (CODEX_ONLY_HARDENED_MODE) {
        return blockDisabledFeature(res, 'Claude session listing');
    }
...
app.get('/api/projects/:projectName/sessions/:sessionId/messages', authenticateToken, async (req, res) => {
    if (CODEX_ONLY_HARDENED_MODE) {
        return blockDisabledFeature(res, 'Claude session history');
    }
...
app.put('/api/projects/:projectName/rename', authenticateToken, async (req, res) => {
    if (CODEX_ONLY_HARDENED_MODE) {
        return blockDisabledFeature(res, 'Project rename');
    }
...
app.delete('/api/projects/:projectName/sessions/:sessionId', authenticateToken, async (req, res) => {
    if (CODEX_ONLY_HARDENED_MODE) {
        return blockDisabledFeature(res, 'Project session deletion');
    }
...
app.delete('/api/projects/:projectName', authenticateToken, async (req, res) => {
    if (CODEX_ONLY_HARDENED_MODE) {
        return blockDisabledFeature(res, 'Project deletion');
    }
...
app.get('/api/browse-filesystem', authenticateToken, async (req, res) => {
    if (CODEX_ONLY_HARDENED_MODE) {
        return blockDisabledFeature(res, 'Filesystem browsing');
    }
...
app.post('/api/create-folder', authenticateToken, async (req, res) => {
    if (CODEX_ONLY_HARDENED_MODE) {
        return blockDisabledFeature(res, 'Folder creation');
    }
...
app.put('/api/projects/:projectName/file', authenticateToken, async (req, res) => {
    if (CODEX_ONLY_HARDENED_MODE) {
        return blockDisabledFeature(res, 'File saving');
    }
...
app.post('/api/projects/:projectName/files/create', authenticateToken, async (req, res) => {
    if (CODEX_ONLY_HARDENED_MODE) {
        return blockDisabledFeature(res, 'File creation');
    }
...
app.put('/api/projects/:projectName/files/rename', authenticateToken, async (req, res) => {
    if (CODEX_ONLY_HARDENED_MODE) {
        return blockDisabledFeature(res, 'File rename');
    }
...
app.delete('/api/projects/:projectName/files', authenticateToken, async (req, res) => {
    if (CODEX_ONLY_HARDENED_MODE) {
        return blockDisabledFeature(res, 'File deletion');
    }
    ...
    if (pathname === '/shell') {
        if (CODEX_ONLY_HARDENED_MODE) {
            ws.send(JSON.stringify({
                type: 'error',
                error: 'Shell access is disabled in Codex-only hardened mode'
            }));
            ws.close();
            return;
        }
...
function handleChatConnection(ws) {
    ...
    ws.on('message', async (message) => {
        try {
            ...
            const defaultProvider = CODEX_ONLY_HARDENED_MODE ? 'codex' : 'claude';
            const provider = data.provider || data.options?.provider || defaultProvider;
            ...
            if (CODEX_ONLY_HARDENED_MODE) {
                ...
                if (!allowedMessageTypes.has(data.type) || !isCodexProviderMessage) {
                    writer.send({
                        type: 'error',
                        error: `${data.type} is disabled in Codex-only hardened mode`
                    });
                    return;
                }
            }
            ...
            if (data.type === 'claude-command') {
                ...
                const activeSessions = CODEX_ONLY_HARDENED_MODE
...
app.post('/api/transcribe', authenticateToken, async (req, res) => {
    if (CODEX_ONLY_HARDENED_MODE) {
        return blockDisabledFeature(res, 'Audio transcription');
    }
...
app.post('/api/projects/:projectName/upload-images', authenticateToken, async (req, res) => {
    if (CODEX_ONLY_HARDENED_MODE) {
        return blockDisabledFeature(res, 'Image upload');
    }
