...
function toCodexOnlyProject(project) {
...
function mergeCodexProjectMetadata(existingMetadata, candidateMetadata) {
  ...
  const candidateHasCustomName = Boolean(candidateMetadata?.isCustomName);
  const existingHasCustomName = Boolean(existingMetadata.isCustomName);

  return {
    name: existingMetadata.name || candidateMetadata?.name || '',
    displayName: candidateHasCustomName && !existingHasCustomName
...
async function buildCodexProjectMetadataLookup(config) {
  ...
  try {
    ...
    for (const entry of directories) {
      ...
      metadataByPath.set(
        ...
        mergeCodexProjectMetadata(metadataByPath.get(normalizedProjectPath), {
          name: entry.name,
    ...
    if (error.code !== 'ENOENT') {
      console.warn('Could not build Codex project metadata from Claude directories:', error.message);
    }
  }

  for (const [projectName, projectConfig] of Object.entries(config)) {
    const configuredPath = projectConfig?.originalPath || projectConfig?.path;
    const normalizedProjectPath = normalizeComparablePath(configuredPath);
    if (!normalizedProjectPath) {
      continue;
    }
...
async function getProjects(progressCallback = null) {
  ...
  if (CODEX_ONLY_HARDENED_MODE) {
    return getCodexOnlyProjects(progressCallback, config, codexSessionsIndexRef);
  }
...
function normalizeComparablePath(inputPath) {
  ...
  try {
    ...
    // Fall back to the normalized absolute path when the target does not exist.
  }
...
async function searchConversations(query, limit = 50, onProjectResult = null, signal = null) {
  ...
  if (CODEX_ONLY_HARDENED_MODE) {
    const codexProjects = (await getProjects()).filter(project => (project.codexSessions || []).length > 0);
    let scannedProjects = 0;
    const totalProjects = codexProjects.length;
    ...
    for (const project of codexProjects) {
      ...
      try {
        ...
        // Skip codex search errors in hardened mode
      }
