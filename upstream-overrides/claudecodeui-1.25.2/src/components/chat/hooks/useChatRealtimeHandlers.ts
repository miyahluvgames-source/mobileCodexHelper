  ...
  useEffect(() => {
    ...
    switch (latestMessage.type) {
      ...
      case 'codex-complete': {
        ...
        const shouldResyncViewedSession =
          Boolean(viewedSessionId) &&
          [codexCompletedSessionId, codexActualSessionId, currentSessionId, selectedSession?.id]
            .filter(Boolean)
            .some((sessionId) => sessionId === viewedSessionId);
        ...
        if (shouldResyncViewedSession) {
          setTimeout(() => onSessionResync?.(), 250);
        }
        break;
      }
